...
int NetworkRequests::handle_get_token_and_hub_id(char* user_id, char* hub_addr, uint16_t* out_hub_id) {
  ...
  if (network->set_power_on_and_wait_for_reg()) {
    ...
    char mutation[len];
    snprintk(mutation, len, "{\"query\":\"mutation loginAsHub{loginAsHub(userId:%s, serial:\\\"%s\\\", imei:\\\"%s\\\")}\",\"variables\":{}}\r", user_id, hub_addr, network->device_imei);
  ...
  ret = -1;
  char query[] = "{\"query\":\"query getHubViewer{hubViewer{id}}\",\"variables\":{}}\r";
...
int NetworkRequests::handle_send_event(char* sensor_addr) {
  ...
  if (network->set_power_on_and_wait_for_reg()) {
    ...
    char mutation[len];
    snprintk(mutation, len, "{\"query\":\"mutation CreateEvent{createEvent(serial:\\\"%s\\\"){ id }}\",\"variables\":{}}\r", sensor_addr);
    cJSON* doc = network->send_request(mutation);
...
int NetworkRequests::handle_add_new_sensor(char* sensor_addr) {
  ...
  if (network->set_power_on_and_wait_for_reg()) {
    ...
    char mutation[len];
    snprintk(mutation, len, "{\"query\":\"mutation createSensor{createSensor(doorColumn: 0, doorRow: 0, isOpen: false, isConnected: true, serial:\\\"%s\\\"){id}}\",\"variables\":{}}\r", sensor_addr);
    cJSON* doc = network->send_request(mutation);
...
int NetworkRequests::handle_update_gps_loc(float lat, float lng, float hdop, float speed, float course) {
  ...
  if (network->set_power_on_and_wait_for_reg()) {
    size_t len = 150;
    char mutation[150];
    snprintk(mutation, len, "{\"query\":\"mutation CreateLocation{createLocation(lat:%.5f, lng: %.5f, hdop: %.2f, speed: %.2f, course: %.2f, age: 0){ id }}\",\"variables\":{}}\r", lat, lng, hdop, speed, course);
    cJSON* doc = network->send_request(mutation);
